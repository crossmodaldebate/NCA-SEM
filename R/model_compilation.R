compile_model - function(model_specification, data) {
  ...
  compiled_model - tryCatch({
    lavaansem(model_specification, sample.cov = cov_matrix, sample.nobs = nrow(data_scaled), start = start_values, fixed.x = FALSE, control = list(maxiter = 1000, trace = 6))
  }, error = function(e) {
...
check_and_adjust_covariance - function(data) {
  ...
  if (!matrixcalcis.positive.definite(cov_matrix)) {
    increment - 1e-6
    while (!matrixcalcis.positive.definite(cov_matrix)) {
...
generate_start_values - function(model_specification, data) {
  ...
  fit - tryCatch(
    {
      lavaansem(model_specification, data = data, do.fit = FALSE)
    },
    error = function(e) {
